...
- `favicon.svg` / `favicon-32.png` / `apple-touch-icon.png`：網站的小圖示（瀏覽器分頁、手機加到主畫面時顯示的icon），木質徽章配鬱金香剪影，跟整體荷蘭鄉村風呼應

## 這個版本新增了什麼（最新一次更新）

- **庫存總覽的統計列**：移除了「🟢 有庫存」這個顯示格（因為它不需要採取任何行動，佔位但沒有實際用途），改成新增「⏳ 快過期」，統計「已過期」＋「2 個月內到期」的品項共有幾項，讓效期資訊在頁面最上方就看得到，不用逐張卡片找。
- **統計列裡的「🟡 低庫存」「🔴 缺貨」「⏳ 快過期」現在都可以直接點擊**：點下去會自動切換到對應的篩選條件（等同於下面「庫存不足／已用完／快過期」篩選鈕的效果），並自動捲動到庫存卡片列表，方便直接找到需要處理的品項，不用自己再點篩選鈕。
- 下方的狀態篩選鈕新增「快過期」選項，跟統計列的點擊行為互相同步（不管從哪裡切換，兩邊的按鈕反白狀態都會一致）。
...
- 庫存總覽卡片與性價比比較的每筆採購，都會直接顯示**備註內容預覽**（不用點開就看得到，適合放採購通路、商品用途、折扣資訊），文字太長會自動裁切，滑鼠移過去（手機長按）可以看到完整內容。
- 加入**閒置自動登出**：預設閒置 60 分鐘沒有任何操作就會自動登出，需要重新輸入信箱密碼。想調整這個時間，改 `app.js` 最上面的 `IDLE_TIMEOUT_MINUTES` 這個數字即可（單位是分鐘，改成 `0` 就是關閉這個機制）。
- **任何分類都可以選填「有效期限」欄位（年月即可，不用到日）**，庫存總覽卡片會依此顯示效期狀態：🗓️ 正常／⏳ 2 個月內到期／⚠️ 已過期。這個欄位一開始只開放給醫療保健用，後來改成所有分類都能用（畢竟保養品、清潔用品其實也有效期限），而且是選填、不強制輸入。
- 性價比比較的分類篩選也比照庫存總覽，從下拉選單改成按鈕。
- **新增「存放地點」欄位**（台北／新竹，按鈕選擇），放在備註欄位上方。**同一品項在不同地點會分開計算庫存**（例如台北跟新竹的洗衣精是各自獨立的庫存，不會互相加總），庫存總覽可以用地點按鈕篩選只看某一邊。
- 庫存總覽的分類篩選改成按鈕（原本是下拉選單），跟存放地點篩選鈕一起放在統計列上方；統計列本身也縮小顯示，讓畫面更精簡。
- 庫存總覽**每頁固定顯示 12 張卡片**，超過會出現「上一頁／下一頁」分頁按鈕。
- 搜尋品名的輸入框，有文字時右側會出現一個 ✕ 清除按鈕，點一下就能清空搜尋重新看全部（庫存總覽、性價比比較兩處搜尋框都有）。
- 性價比比較的每一筆採購，都會顯示**採購人**。

## 關於「存放地點」的資料相容性，以及一個已修復的 bug

存放地點是新加的欄位，**在這之前已經存在的舊採購紀錄還沒有地點資料**。之前的版本有個 bug：只要編輯這種舊紀錄（哪怕只是想補個備註），因為表單一定要選一個地點、預設會帶入「台北」，導致這筆採購被系統當成「換了一個新品項」，使用紀錄留在舊的那邊，變成兩張卡片、舊卡片庫存還可能顯示負數——**這個問題已經修好了**：現在編輯時如果偵測到「品項身分改變了」（不管是改品名、最小單位、還是這次新加的地點），只要這是那個舊身分底下唯一一筆採購，系統會自動把完整的使用／調整歷史一起搬到新身分底下，不會再分裂成兩張卡片。

⚠️ 如果同一個品項本來就有**兩筆以上不同的採購批次**，编辑其中一筆造成身分改變時，系統不會自動搬動全部歷史（因為使用紀錄可能是好幾批一起共用的，搬錯了反而更亂），這種比較少見的情況還是需要你確認一下庫存數字對不對。

另外，**第一次編輯舊資料時，地點會被預設猜成「台北」**，如果那個品項其實放在新竹，記得手動改成正確的地點再儲存。

**另一個已修復的 bug：** 如果某個品項的「使用」紀錄日期比「採購」紀錄還新（例如採購是去年，使用是 3 個月前），卡片會一直顯示「📍 未設定地點」，就算重新編輯採購、選好地點存檔也沒用。原因是卡片顯示用的地點，原本是抓「日期最新的那一筆異動」，但「使用」紀錄本身從來不會記錄地點，所以只要有一筆使用紀錄的日期比採購新，就會把地點顯示蓋成空白，而且編輯採購也修不好（因為問題出在那筆使用紀錄，不是採購紀錄）。現在已經改成**只以「採購」紀錄來決定地點顯示**，不會再被使用紀錄影響，這個是純粹的顯示邏輯修正，不需要改動任何既有資料。
