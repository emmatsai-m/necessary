...
- **分類選「醫療保健」時會多一個「有效期限」欄位**，庫存總覽卡片會顯示效期狀態：🗓️ 正常／⏳ 30 天內到期／⚠️ 已過期。
...
存放地點是新加的欄位，**在這之前已經存在的舊採購紀錄還沒有地點資料**，庫存總覽上會顯示「📍 未設定地點」，並且會被歸類成獨立的一組庫存（不屬於台北，也不屬於新竹，所以地點篩選鈕篩「台北」或「新竹」時不會看到它們，只有篩「全部」才看得到）。只要之後編輯那筆採購紀錄、選一個地點存檔，就會自動歸類過去——這跟前面提到「修改品名或最小單位會被當成新品項」是同一種機制，所以也一樣建議：如果要幫舊資料一次補上地點，麻煩耐心一筆一筆編輯儲存；如果數量比較多想要我幫忙批次處理，也可以再跟我說。
